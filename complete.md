...
*Notebook state as of 2026-09-21. For the backend and hosting, see §11ag.*
...
## 11ag. Hosting, the angles-only public API, and ops (2026-09-25)

Branch `backend/upload-endpoint-and-local-hosting`, pushed; last commit
`48132e7`. PR not opened (`gh` is not installed) — open it from
`https://github.com/umar551869/Creative-Angle-Review-Agent/compare/main...backend/upload-endpoint-and-local-hosting?expand=1`.

### What is running, and where

| Port | What | Reachable from |
|---|---|---|
| **8000** | Docker container `backend-api-1` — **the live backend** | public URL, LAN, this PC |
| **8001** | local `Backend\.venv` uvicorn — testing only | this PC only |

- **Public URL (fixed): `https://atlantic-canine-hurling.ngrok-free.dev`** —
  a free ngrok static domain. The authtoken is in
  `%LOCALAPPDATA%\ngrok\ngrok.yml`, never in the repo. Every request should
  send `ngrok-skip-browser-warning: 1`.
- `Backend\tools\start_local.ps1` brings back Docker (:8000), the local
  backend (:8001) and ngrok after a reboot. The URL does not change.
- The old trycloudflare tunnel (`pac-night-haven-tom…`) is **still running**
  only until the partner's Vercel app is switched to the ngrok URL — then stop
  `cloudflared`. Quick tunnels expire silently and change hostname on restart;
  that is why ngrok replaced it.

**The trap that cost hours:** a leftover local uvicorn bound to
`127.0.0.1:8000` beats Docker's `0.0.0.0:8000` bind on Windows. The tunnel
(pointed at 127.0.0.1:8000) silently served the local process while Docker
looked healthy — two "Docker" audits were actually local runs. **Never start
the local backend on 8000.** To see what the public URL serves, read `/ready`'s
`ffmpeg` path: `/usr/bin/ffmpeg` is Docker, a `C:\…` path is the local one.

### The public API returns only creative angle → video links

The user's requirement: the frontend sends one brief + many video links, gets
back **which creative angle each video falls under**; scores, verdicts and
...
- A request that came **through the tunnel** (`X-Forwarded-For` /
  `Cf-Connecting-Ip` present) gets `view: "angles"`: `results`, `brief`,
  `timings` emptied; `/jobs/{id}/report/…` and `reports.zip` return **403**.
  A request on the host gets everything. `AUDITOR_PUBLIC_VIEW=full` undoes it.
- Code: `pipeline.angle_groups()` in `app/services/pipeline.py`;
  `_through_tunnel()` / `_angles_only()` in `app/main.py`.
- Also fixed on the way: links that failed to download used to vanish from the
  answer (now in `unplaced` with the reason, or the job's `error` when the whole
  job failed); `vm.tiktok.com` / `/t/` short links are resolved before download
  (`ingest.resolve_short_link`) so parallel downloads cannot claim each other's
  file, and results still name the link the caller sent.
- Tests: `Backend/tests/test_angle_groups.py`. Suite: **204 passed, 1 skipped**.
...
First run of 3 videos: brief 52 s · download 35 s · decode 53 s ·
**speech/OCR 583 s** · vision 163 s · judging 114 s = **16.7 min**, i.e.
~5 min per video. Cached re-runs are much faster. CPU sits at ~400% (of 800%,
8 logical cores) during phase 2; memory ~3 of 8 GB. The frontend doc says
"about 5 minutes per video".

### Settings the user decided

- **`AUDITOR_ALWAYS_RECOMPILE_BRIEF` stays ON** (user's call). Consequence,
  seen live: the same video scored 63, 67 and 84 on different runs because the
  brief compiles differently each time. Angle grouping was stable across runs.
- **TikTok cookies:** `Backend\cookies.txt` (git- and docker-ignored). `.env`
  points the local backend at it; `docker-compose.yml` mounts it at
  `/app/cookies.txt` and overrides `AUDITOR_COOKIES_FILE`.
- `wordninja` added to `requirements.txt` (was silently missing from the image).
  It did not fix the OCR errors seen (`SUPPORTSA`) — those are misread
  letters, not missing spaces.

### Docker build

- torch is its own layer before `requirements.txt` is copied; wheels come from a
  BuildKit pip cache mount; `BAKE_MODELS=false` (the `audit-models` volume
  already holds the weights). An app-code rebuild measured **18 s**; before,
  one new package re-downloaded everything (over an hour on this line).
- This network's Docker connection is intermittently flaky ("Connection refused"
  to deb.debian.org, "no such host" for Docker Hub) — a retry usually works.
- `tools/run_job.py` with no flags now **prompts** for the links and runs an
  audit in-process (no server needed).

### Frontend handoff

`Backend/docs/06_frontend_integration.md` is the partner's contract: endpoint
table matching the real routes (and what is 403 through the public URL), §3b
exact input/output for `angles`, a render example, updated checklist. The API
key goes to the partner by direct message, never in the repo.
...
- **Windows, PowerShell 5.1.** No `&&`/`||` chaining — use `;` and `if ($?)`.
  **A git repository** (github.com/umar551869/Creative-Angle-Review-Agent).
- **The backend is live on this PC** — Docker :8000 behind the ngrok URL, local
  `.venv` on :8001. Read §11ag before touching ports, tunnels or the API shape.
